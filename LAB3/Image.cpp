...
void Image::scale(int bit) {
  ...
  for(int rows = 0; rows < nrows; rows ++) {
    for(int cols = 0; cols < ncols; cols ++) {
      ...
      setPix(rows, cols, scaled_val);  // goruntunun piksel degerleri ayarlandi
    }
  }
}
...
Image Image::idft2(Complex** F) {
  ...
  for(int x = 0; x < M; x++) {
    for(int y = 0; y < N; y++) {
      ...
      temp.setPix(x, y, complex_val.abs()); // olusturulan goruntuye complex f(x, y) degerlerinin genligi atandi
    }
  }
